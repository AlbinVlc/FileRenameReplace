...
//	check_admin_referer('imageman');
	...
	if(!empty($_POST) && $_POST['action']=='Guardar') {
		$basedir = $updir['basedir'];

		$new_fileids   = isset($_POST['fileid'])   ? $_POST['fileid']   : [];
		$new_postdates = isset($_POST['postdate']) ? $_POST['postdate'] : [];
		$new_filepaths = isset($_POST['filepath']) ? $_POST['filepath'] : [];
		$new_filenames = isset($_POST['filename']) ? $_POST['filename'] : [];
		$new_files     = isset($_FILES['newfile']) ? $_FILES['newfile'] : [];

		$max = min(count($new_fileids), count($new_filepaths), count($new_filenames));
		for($itr=0; $itr<$max; $itr++) {
			$new_fileid   = intval($new_fileids[$itr]);
			$new_postdate = trim($new_postdates[$itr]);
			$new_filepath = trim($new_filepaths[$itr]);
			$new_filename = trim($new_filenames[$itr]);
			...
			if($old_filepath!==$new_filepath || $old_filename!==$new_filename) {
				$this->rename_media($new_fileid, $new_filepath, $new_filename, $err);
			}
			if($postdate!==$new_postdate) {
	...
	if(!empty($_POST) && $_POST['action']=='Eliminar') {
		$basedir = $updir['basedir'];
		...
		for($itr=0; $itr<$max; $itr++) {
			...
			foreach($ids_rows as $row) {
				...
				if(wp_delete_attachment($element_id, true)===FALSE) echo 'Failed '.$element_id.'<br/>';
			}
...
<?
	if($iids===NULL) {
		...
		?><form action="tools.php?page=filerenamereplace&iid=<?=urlencode(implode(chr(44), $iids)) ?>" method="POST" enctype="multipart/form-data"><?
		foreach($iids as $idx=>$iid) {
			$post     = get_post($iid);
			if($post===NULL) {
				continue;
			} elseif($idx===0 && count($iids)!==1) {
				?><input class="button button-primary" type="submit" name="action" value="Guardar" /><?
				if(count($iids)===1) {
					?> <input class="button button-secondary" type="submit" name="action" value="Eliminar" /> <?
				}
			}
			$postdate = $post->post_date_gmt;
